...
public class MainActivity extends Activity {
    ...
    private void render(ViewGroup root, JSONObject child) throws Exception {
        ...
        LinearLayout.LayoutParams layoutParams = new LinearLayout.LayoutParams(
            style.has("width") ? Integer.parseInt(style.getString("width")) :
                (root instanceof FrameLayout || ((LinearLayout)root).getOrientation() == LinearLayout.VERTICAL ? LinearLayout.LayoutParams.MATCH_PARENT : LinearLayout.LayoutParams.WRAP_CONTENT),
            style.has("height") ? Integer.parseInt(style.getString("height")) :
                (root instanceof FrameLayout ? LinearLayout.LayoutParams.MATCH_PARENT : LinearLayout.LayoutParams.WRAP_CONTENT)
        );
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        try {
            ...
            if (head.has("icon")) getActionBar().setIcon(new BitmapDrawable(getResources(), fetchBitmap(head.getString("icon"))));

            JSONArray children = page.getJSONArray("body");
            for (int i = 0; i < children.length(); i++) render(root, children.getJSONObject(i));
        } catch (Exception e) {
            Log.d("jn", Log.getStackTraceString(e));
        }
    }
